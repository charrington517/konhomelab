...
app.post('/api/test-service', async (req, res) => {
  try {
    const { type } = req.body;
    const config = loadConfig();
    let result = { ok: false, message: 'Unknown service type' };
    ...
    switch (type) {
      case 'proxmox':
        if (config.proxmox?.enabled) {
          try {
            ...
            result = { ok: false, message: `Tdarr connection failed: ${err.message}` };
          }
        } else {
          result = { ok: false, message: 'Proxmox not enabled' };
        }
        break;
      ...
      case 'unraid':
        if (config.unraid?.enabled) {
          ...
          result = { ok: false, message: 'Unraid not enabled' };
        }
        break;
      ...
      case 'sonarr':
        if (config.media?.sonarr?.enabled) {
          try {
            const response = await axios.get(, {
              headers: { 'X-Api-Key': config.media.sonarr.apiKey },
              timeout: 5000
            });
            result = { ok: true, message:  };
          } catch (err) {
          ...
          result = { ok: false, message: 'Sonarr not enabled' };
        }
        break;
      ...
      case 'radarr':
        if (config.media?.radarr?.enabled) {
          try {
            const response = await axios.get(, {
              headers: { 'X-Api-Key': config.media.radarr.apiKey },
              timeout: 5000
            });
            result = { ok: true, message:  };
          } catch (err) {
          ...
          result = { ok: false, message: 'Radarr not enabled' };
        }
        break;
      ...
      case 'qbittorrent':
        if (config.qbittorrent?.enabled) {
          ...
          result = { ok: false, message: 'qBittorrent not enabled' };
        }
        break;
      ...
      case 'prowlarr':
        if (config.prowlarr?.enabled) {
          ...
          result = { ok: false, message: 'Prowlarr not enabled' };
        }
        break;

      case 'tdarr':
        if (config.tdarr?.enabled) {
          try {
            const response = await axios.get(, {
              timeout: 5000
            });
            result = { ok: true, message: 'Connected to Tdarr server' };
          } catch (err) {
            result = { ok: false, message: `Tdarr connection failed: ${err.message}` };
          }
        } else {
          result = { ok: false, message: 'Tdarr not enabled' };
        }
        break;
    }
